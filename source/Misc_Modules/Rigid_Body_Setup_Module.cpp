...
void Rigid_Body_Setup_Module::M_on_parent_object_set()
{
    ...
    glm::vec3 random_velocity = LEti::Math::random_vec3(m_min_velocity, m_max_velocity);
    rb->set_velocity(random_velocity);

    constexpr glm::vec3 Limit_Vector = {1.0f, 1.0f, 1.0f};
    glm::vec3 random_angular_velocity = LEti::Math::random_vec3(-Limit_Vector, Limit_Vector);
    float random_angular_velocity_value = LEti::Math::random_number_float(m_min_angular_velocity, m_max_angular_velocity);
    LEti::Math::extend_vector_to_length(random_angular_velocity, random_angular_velocity_value);
